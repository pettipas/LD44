...
public static class Extensions {
    ...
    public static int GetPositionKey(this GameObject g) {
        Vector3 pkey = g.transform.position.Round();
        float signx = Mathf.Sign(pkey.x);
        float signy = Mathf.Sign(pkey.y);
        float signz = Mathf.Sign(pkey.z);

        if (signx > 0 && signy > 0 && signz > 0) {
            string s = pkey.x + "" + pkey.y + "" + pkey.z;
            return int.Parse(s);
        }
        return 000;
    }
    ...
    public static void SafePlay(this Animator animator, string a, int layer = 0, float time = 0) {
        ...
        try {
            ...
            Debug.Log(e.StackTrace + a);
        }
        
    } 

    /// <summary>
    /// Explicitly exits from state and enters to state
    /// </summary>
    ...
    /// <summary>
    /// Safely exits all states and goes to this state
    /// </summary>
    /// <param name="mb">Mb.</param>
    public static void GotoState(this MonoState mb) {
        MonoState[] states = mb.GetComponentsInChildren<MonoState>();
        for (int i = 0; i < states.Length; i++) {
            if (states[i] != mb) {
                states[i].SetStateInActive();
    ...
    public static void SetStateActive<T>(this T mono) where T : MonoState {
        if (mono != null && !mono.enabled) {
            mono.enabled = true;
        }
    }

    public static void SetStateInActive<T>(this T mono) where T : MonoState {
        if (mono != null && mono.enabled) {
            mono.enabled = false;
        }
    }
    ...
    public static Vector3 NSEW(this Vector3 vect)
    {
        float sx = Mathf.Sign(vect.x);
        float sy = Mathf.Sign(vect.y);
        float x = Mathf.Abs(vect.x) > 0 ? 1 : 0;
        float y = Mathf.Abs(vect.y) > 0 ? 1 : 0;
        ...
        return new Vector3(x*sx, y*sy, Mathf.RoundToInt(vect.z));
    }

    public static Vector3 NSEWXZ(this Vector3 vect) {
        float x = Mathf.Round(vect.x * 2f) * 0.5f;
        float z = Mathf.Round(vect.z * 2f) * 0.5f;

        return new Vector3(x, Mathf.RoundToInt(vect.y), z);
    }
